...
class Base extends Controller {
	...
	protected function initialize(){
		...
		//未登陆，不允许直接访问
        if (!defined('__ROOT__')) {
            $root = rtrim(dirname(rtrim(request()->baseFile(), '/')), '/');
            define('__ROOT__', (('/' == $root || '\\' == $root) ? '' : $root));
        }
        if (request()->isMobile()){
            Session::clear();
            Cache::clear();
            config('app.default_ajax_return','json');
            $this->error('暂不支持手机登陆',url('index/Login/login'));
        }
		...
		if (!session('?user_id')){
		    ...
		    Cache::clear();
		    config('app.default_ajax_return','json');
		    $this->error('请登录账号',url('index/Login/login'));
		}
 		if(time()>($login_time+intval(config('login.login_config.effective_time')*60))){
 		    Session::clear();
 		    Cache::clear();
 		    config('app.default_ajax_return','json');
 		    $this->error('请重新登录',url('index/Login/login'));	   
		}
