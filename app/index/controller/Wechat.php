...
class Wechat extends Base{
    ...
    protected function initialize(){
        ...
        if (!$wechat_info){
            ...
            $this->error('请前往系统管理配置微信公众号',url('index/Sys/wechatList'));
        }
    }
    /**
     * 菜单列表
    ...
    public function menuList(){
        ...
        $wechat_menu=new \app\index\model\WechatMenu;
        $menu_list = $wechat_menu->getList($menu_condition);
        $show = $wechat_menu->page($page_condition,$page);
    ...
    public function editMenu(){
        $id=input('id');
        $wechat_menu=new \app\index\model\WechatMenu;
        $return_data=$wechat_menu->getEditMenu($id,$this->uid);
    ...
    public function saveMenu(){
        ...
        return $this->saveData('wechat_menu', $data,$rule,new_url('index/Wechat/menuList'));  
    }
    ...
    public function delMenu(){
        //删除子级菜单
        $ids=input('ids/a');
        $menu_data=db('wechat_menu')->where('id','in',$ids)->field('pid,id')->select();
        if ($menu_data){
            $menu_ids=[];
            ...
            if (!empty($menu_ids)){
                $del_ids=array_diff($menu_ids, $ids);
                if ($del_ids){
                    $del_result = db('wechat_menu')->delete($del_ids);
                }
            }
        }
