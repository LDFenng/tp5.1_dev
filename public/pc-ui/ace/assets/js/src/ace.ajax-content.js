...
(function($ , undefined) {
	...
	function AceAjax(contentArea, settings) {
		...
		this.loadUrl = function(hash, cache, manual_trigger) {
			...
			hash = hash.replace(/^(\#\!)?\#/, '');
			if(hash.indexOf('?')!=-1){
			var path_info = hash.split('?');
				hash = path_info[0];
				param_info = path_info[1];
			}
			this.force_reload = (cache === false)
			
			if(typeof this.settings.content_url === 'function') url = this.settings.content_url(hash);
			if(typeof url === 'string') this.getUrl(url, hash, manual_trigger,'GET',param_info);
		}
		...
		this.loadAddr = function(url, hash, cache) {
			...
			this.force_reload = (cache === false);
			this.getUrl(url, hash, false,'GET',param_info);
		}
...
//			}
		...
		this.getUrl = function(url, hash, manual_trigger, method, data, updateView, extraParams) {
			...
			var ajax_params = method == 'GET' ? {'url': url, 'cache': !this.force_reload,'data':data} : {'url': url, 'method' : 'POST', 'data': data}
			...
			.done(function(result) {
				if(result.code==0){
			        layer.confirm(result.msg, {icon: 5}, function (index) {
			            layer.close(index);
				...
				if(result.match(re)) colseload();//self.stopLoading();
				else colseload();//self.stopLoading(true);
			})
		}
